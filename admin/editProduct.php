...
if(isset($_POST['productName']))
{
    ...
    if($model->edit("productID=$id",$_POST,true))echo "<script>alert('添加成功!');</script>";
    echo "<script>window.location.replace('productList.php');</script>";
    exit(0);
}
echo file_get_contents('header.html');
$data = $model->getList(['*'],'productId = ' . $_GET['productId']);
$data = $data[0];
?>
<script>
    var details = [];
    ...
    <form action="editProduct.php" method="post">
        <div class="container-fluid">
            <div class="form-group">
                ...
                <input name="productName" id="productName" placeholder="修马桶" class="form-control" value="<?php echo $data['productName'];?>">
            </div>
            <div class="form-group">
                <label for="orderAmount">近期销量(初始)</label>
                <input name="orderAmount" id="orderAmount" placeholder="100" class="form-control" value="<?php echo $data['orderAmount'];?>">
            </div>
            <div class="form-group">
                <label for="duration">服务耗时(分钟)</label>
                <input name="duration" id="duration" placeholder="120" class="form-control" value="<?php echo $data['duration'];?>">
            </div>
            <div class="form-group">
                <label for="abstract">摘要(显示在产品列表)</label>
                <input name="abstract" id="abstract" placeholder="专业修马桶1000年" class="form-control" value="<?php echo $data['abstract'];?>">
            </div>
            <div class="form-group">
                <label for="description">描述</label>
                <input name="description" id="description" placeholder="专业修马桶1000年" class="form-control" value="<?php echo $data['description'];?>">
            </div>
            <div class="form-group">
                <label for="latitude">图文详情</label>
                <div id="detailsView" class="">
                    <?php
                        ...
                        {
                            if($value['type'] == '1')
                                echo "<p><img src='$value[content]'></p>";
                            else
            ...
            <div class="form-group">
                ...
                <input name="originalPrice" id="originalPrice" placeholder="100" class="form-control"  value="<?php echo $data['originalPrice'];?>">
            </div>
            <div class="form-group">
                <label for="settlePrice">真实价格(元)</label>
                <input name="settlePrice" id="settlePrice" placeholder="50" class="form-control"  value="<?php echo $data['settlePrice'];?>">
            </div>
            <div class="form-group">
                <label for="minOrders">最小下单份数</label>
                <input name="minOrders" id="minOrders" placeholder="1" class="form-control"  value="<?php echo $data['minOrders'];?>">
            </div>
            <div class="form-group">
                <label for="maxOrders">最大下单份数</label>
                <input name="maxOrders" id="maxOrders" placeholder="1" class="form-control"  value="<?php echo $data['maxOrders'];?>">
            </div>
            <div class="form-group">
                <label for="imageUrls">多张产品图</label>
                <div id="imagesView" class="">
                    <?php
                    $array = json_decode($data['imageUrls'],true);
                    if(is_array($array))
                    foreach($array as $value)
                    {
                        echo "<img src='$value'>";
                ...
                <input name="imageUrls" id="imageUrls" placeholder="json数据,请勿修改!" class="form-control" value="<?php echo htmlspecialchars($data['imageUrls']);?>">
            ...
            <div class="form-group">
                ...
                <div id="imagesView2" class="">
                    <?php echo "<img src='$data[thumbUrl]'>";?>
                </div>
                <input name="thumbUrl" id="thumbUrl" placeholder="json数据,请勿修改!" class="form-control" value="<?php echo $data['thumbUrl'];?>">
            ...
            <div class="form-group">
                ...
                <input name="rank" id="rank" placeholder="1" class="form-control" value="<?php echo $data['rank'];?>">
            </div>
            <div class="form-group">
